...
了解。  
このラボは **「フロントエンドのアクセス制御を、バックエンドの X-Original-URL 処理でバイパスできてしまう脆弱性」**。  
面接でそのまま使えるように、**脆弱性の本質 → 気づき方 → 攻撃手順 → 対策 → 30秒版**を一気にまとめるよ。
...
> この脆弱性は、フロントエンドとバックエンドのアクセス制御が不一致なことで発生します。  
> フロント側は `/admin` をブロックしていますが、バックエンドは `X-Original-URL` を優先してルーティングするため、ヘッダを書き換えるだけで管理画面にアクセスできてしまいます。  
> 攻撃者は `X-Original-URL: /admin/delete` のように送ることで管理操作を実行できます。  
> 対策は、バックエンドでも必ず認可チェックを行い、信頼できないヘッダを削除することです。
