...
このラボは PortSwigger が「4桁の 2FA コード」を使う仕様になってる。

理由はシンプルで：

• 画面に「Enter 4‑digit code」などの UI がある
• 実際のリクエストでも mfa-code=1234 のように 4桁で送られている
• ラボ説明文にも「0000〜9999 を総当たり」と書かれている


つまり アプリ側が 4 桁の数字を要求しているから。
...
🔥 ② なぜ mfa-code を攻撃できるのか？（ここが本質）

理由は 2FA の “所有者” と “ログイン対象アカウント” の紐付けが壊れているから。

普通の安全な 2FA はこうなる：

• verify=wiener
• mfa-code は wiener のコード
• だから wiener のコードしか通らない
...
🟥 壊れているポイント（Broken Logic）

サーバーはこう判断してしまっている：

• verify=◯◯ の値だけを見て
• 「この人の 2FA コードをチェックしよう」と決めている
• でも その verify が本当にログイン中のユーザーか確認していない
...
ログイン中のユーザー：wiener
verify=carlos
mfa-code=（wiener のコード or 総当たり）


これで carlos としてログインできてしまう。

---

🔥 ③ なぜ mfa-code を総当たりできるのか？

理由は：

• 2FA コードが 4桁の数字だけ
• つまり 0000〜9999 の 1 万通りしかない
• ロックアウトもレート制限もない
• verify=carlos に書き換えると carlos のコードが生成される


だから Intruder で総当たりできる。
...
なぜ 4 桁か？
→ アプリが 4 桁の 2FA を採用しており、リクエストも 4 桁で送られているため。

なぜ mfa-code を攻撃できるのか？
→ 2FA の verify パラメータがログイン中のユーザーと紐付いておらず、
　攻撃者が verify=carlos に書き換えることで
　「carlos の 2FA コードを総当たりできる」broken logic があるため。
