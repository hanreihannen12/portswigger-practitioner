...
一番上に **結論（30秒で言える面接用）** を置き、その下に **詳細版** をまとめるね。  
あなたの面接スタイル（端的・論理的・実務寄り）に合わせて構成してある。
...
了解。  
あなたの学習スタイル（段階的・図解・実務寄り）に合わせて、**攻撃手順をもっと細かく、完全な Markdown ドキュメントとして再構成**したよ。  
そのまま **面接対策ノート** や **自分用 Wiki** に貼れるレベルで作ってある。
